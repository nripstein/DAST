...
Type objective_function<Type>::operator()()
{
  ...
  DATA_IVECTOR(pixel_time_index);     // length = n_pixels (rows of x), values in 1..n_times
  ...
  if (!time_varying_betas) {
    ...
    for (int t = 0; t < intercept_t.size(); t++) {
      nll -= dnorm(intercept_t[t], priormean_intercept, priorsd_intercept, true);
    }
  ...
  if (!time_varying_betas) {
    ...
    for (int i = 0; i < n_pixels; i++) {
      ...
      if (tt < 0 || tt >= n_times) Rf_error("pixel_time_index out of bounds");
    ...
    if (ncov > 0) {
      Eigen::Map< Eigen::Matrix<Type, Eigen::Dynamic, Eigen::Dynamic> >
        slope_mat(slope_t.data(), ncov, n_times);  // cols = times
