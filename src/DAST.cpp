...
template <class Type> Type objective_function<Type>::operator()() {
  ...
  // The A matrices are for projecting the mesh to a point for the pixel and
  ...
  // Two-column integer array with start and end indices (already computed in
  // R).
  ...
  // ------------------------------------------------------------------------ //
  // Data for time-varying betas
  // ------------------------------------------------------------------------ //
  DATA_INTEGER(time_varying_betas); // 0/1 flag
  DATA_IVECTOR(
      pixel_time_index); // length = n_pixels (rows of x), values in 1..n_times
  DATA_INTEGER(n_times); // T
  DATA_INTEGER(ncov);    // p (ncols of x)

  // ------------------------------------------------------------------------ //
  // Parameters
  // ------------------------------------------------------------------------ //

  // Pooled beta parameters
  PARAMETER(intercept);
  PARAMETER_VECTOR(slope);

  // Time-varying beta parameters
  PARAMETER_VECTOR(intercept_t); // length n_times
  PARAMETER_VECTOR(slope_t);     // length ncov * n_times (flattened)
  ...
  if (pixel_time_index.size() != n_pixels) {
    Rf_error("pixel_time_index length (%d) != n_pixels (%d)",
             (int)pixel_time_index.size(), (int)n_pixels);
  }
  if (ncov != x.cols()) {
    Rf_error("ncov (%d) != ncol(x) (%d)", (int)ncov, (int)x.cols());
  }
  if (n_times < 1) {
    Rf_error("n_times must be >= 1");
  }
  ...
  if (!time_varying_betas) {
    ...
    // Time-varying betas: element-wise priors
    for (int t = 0; t < intercept_t.size(); t++) {
      nll -=
          dnorm(intercept_t[t], priormean_intercept, priorsd_intercept, true);
    }
    for (int k = 0; k < slope_t.size(); k++) {
      nll -= dnorm(slope_t[k], priormean_slope, priorsd_slope, true);
    }
  }
  ...
  if (iid && family != 3) {
    ...
    Type log_pcdensity_iid = log(lambda / 2) - (3.0 / 2.0) * iideffect_log_tau -
                             lambda * pow(iideffect_tau, -1.0 / 2.0);
  ...
  if (field) {
    ...
    // From https://www.tandfonline.com/doi/full/10.1080/01621459.2017.1415907
    // (Theorem 2.6)
    ...
    // From Lindgren (2011) https://doi.org/10.1111/j.1467-9868.2011.00777.x,
    // see equation for the marginal variance
    Type scaling_factor = sqrt(exp(lgamma(nu)) / (exp(lgamma(nu + 1)) * 4 *
                                                  M_PI * pow(kappa, 2 * nu)));
  ...
  if (family == 3) { // Exponential PC prior on tau using tau^2 AUG 25
    ...
    // log pi(iideffect_log_tau) = log(lambda) + iideffect_log_tau - lambda *
    // exp(iideffect_log_tau)
    Type log_pi_logtau =
        log(lambda_nb) + iideffect_log_tau - lambda_nb * exp(iideffect_log_tau);
    nll -= log_pi_logtau;
  }
  ...
  if (!time_varying_betas) {
    ...
    for (int i = 0; i < n_pixels; i++) {
      int tt = pixel_time_index[i] - 1;
      if (tt < 0 || tt >= n_times)
        Rf_error("pixel_time_index out of bounds");
      pixel_linear_pred[i] = intercept_t[tt];
    }

    // time-specific slopes
    if (ncov > 0) {
      Eigen::Map<Eigen::Matrix<Type, Eigen::Dynamic, Eigen::Dynamic>> slope_mat(
          slope_t.data(), ncov, n_times); // cols = times

      for (int i = 0; i < n_pixels; i++) {
        int tt = pixel_time_index[i] - 1; // 0-based column
        // 1xP · P×1 => scalar
        Type cov_dot = x.row(i).dot(slope_mat.col(tt));
        pixel_linear_pred[i] += cov_dot;
      }
    }
  }
  ...
  for (int polygon = 0; polygon < n_polygons; polygon++) {
    ...
    pixel_pred =
        pixel_linear_pred
            .segment(start_end_index(polygon, 0), start_end_index(polygon, 1))
            .array();
    if (iid && family != 3) {
    ...
    numerator_pixels = pixel_pred * aggregation_values
                                        .segment(start_end_index(polygon, 0),
                                                 start_end_index(polygon, 1))
                                        .array();
    normalisation_pixels = aggregation_values.segment(
        start_end_index(polygon, 0), start_end_index(polygon, 1));
    ...
    if (family == 0) {
      polygon_sd = gaussian_sd *
                   sqrt((normalisation_pixels * normalisation_pixels).sum()) /
                   normalisation_total;
      ...
      nll -= dnorm(normalised_polygon_response, pred_polygonrate, polygon_sd,
                   true);
      reportnll[polygon] = -dnorm(normalised_polygon_response, pred_polygonrate,
                                  polygon_sd, true);
    } else if (family == 1) {
      nll -= dbinom(polygon_response_data[polygon],
                    response_sample_size[polygon], pred_polygonrate, true);
      reportnll[polygon] =
          -dbinom(polygon_response_data[polygon], response_sample_size[polygon],
                  pred_polygonrate, true);
    } else if (family == 2) {
      nll -= dpois(polygon_response_data[polygon], pred_polygoncases, true);
      reportnll[polygon] =
          -dpois(polygon_response_data[polygon], pred_polygoncases, true);
    } else if (family == 3) {
